...
const Navbar = ({ setCategory, category, setKeyword }) => {
  ...
  return (
    ...
    <div className='NavBar text-lg md:text-2xl ps-3 pt-2 grid items-center border-slate-800 border-b-2 sticky top-0 bg-[#0f172abf]  ' style={{ gridTemplateColumns: 'auto 1fr auto', height: '80px' }}>
       <div className="xl:hidden flex items-center pr-3">
      ...
      <div className="hidden xl:flex gap-x-2 px-6 text-lg cursor-pointer justify-center  xl:gap-x-4 2xl:gap-x-6">
      <Navitem text= "Home" icon={<IoHome size={25}/>} onclick={()=>handleClick('general')} isActive={category ==='general'}/>
      <Navitem text= "Sports" icon={<MdOutlineSportsCricket size={25}/>} onclick={()=>handleClick('general')} isActive={category ==='sports'}/>
      <Navitem text= "Science" icon={<SiBookstack size={25}/>} onclick={()=>handleClick('general')} isActive={category ==='science'}/>
      <Navitem text= "Entertainment" icon={<PiTelevisionSimpleFill size={25}/>} onclick={()=>handleClick('general')} isActive={category ==='entertainment'}/>
      <Navitem text= "Technology" icon={<HiOutlineDesktopComputer size={25}/>} onclick={()=>handleClick('general')} isActive={category ==='technology'}/>
      <Navitem text= "Business" icon={<IoBusiness size={25}/>} onclick={()=>handleClick('general')} isActive={category ==='business'}/>
      <Navitem text= "Health" icon={<CiHospital1 size={25}/>} onclick={()=>handleClick('general')} isActive={category ==='health'}/>
      </div>

      <div className="flex items-center bg-[#0f172abf] px-3 justify-start ">
        <AiOutlineSearch size={25} className='text-slate-300 mr-2' />
        <input type="search" placeholder="Search" className="w-80 bg-[#0f172abf] focus:outline-none border-none justify-center rounded text-slate-300" value={inputValue} onChange={handleChange}
        />
   ...
   {menuOpen && (
     <div className="xl:hidden absolute top-20 left-0 right-0 bg-[#0f172abf] p-4 z-20">
     <Navitem text="Home" icon={<IoHome size={25} />} onClick={() => handleClick('general')} isActive={category === 'general'} />
